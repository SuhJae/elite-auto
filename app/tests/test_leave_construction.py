...
def build_context(states: list[ShipState], ship_control: FakeShipControl, input_adapter: FakeInputAdapter) -> Context:
    logger = logging.getLogger("elite_auto.test_leave_construction")
...
class TestLeaveConstruction(unittest.TestCase):
    def test_leave_construction_runs_auto_launch_reverse_then_stop(self) -> None:
    ...
    def test_leave_construction_rejects_already_undocked(self) -> None:
        ship_control = FakeShipControl()
        input_adapter = FakeInputAdapter()
    ...
    def test_leave_construction_releases_thrust_when_mass_lock_wait_fails(self) -> None:
        ship_control = FakeShipControl()
        input_adapter = FakeInputAdapter()
        context = build_context(
            [
